...
use crate::tftp::core::{Packet, TransferOption, OptionType};
...
impl Client {
    ...
    pub fn new(config: ClientConfig) -> anyhow::Result<Self> {
        let server_str = config.server.ok_or_else(|| anyhow::anyhow!("Server address not specified"))?;
        let server_ip: IpAddr = server_str.parse()
        ...
        loop {
            ...
            match socket.recv_from(&mut buf) {
                Ok((amt, src)) => {
                    ...
                    match packet {
                        Packet::Data { block_num: block, data } => {
                            if block == block_num {
                                file.write_all(&data)?;
