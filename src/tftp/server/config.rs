...
impl Config {
    ...
    pub fn merge_cli(
        ...
        if self.ip.is_none() { self.ip = Some(cli_ip); }
        if self.port.is_none() { self.port = Some(cli_port); }
        if self.directory.is_none() { self.directory = Some(cli_path); }
        if self.read_only.is_none() { self.read_only = Some(cli_read_only); }
        if self.single_port.is_none() { self.single_port = Some(cli_single_port); }
        
        // Set defaults for others if not present
        if self.overwrite.is_none() { self.overwrite = Some(true); }
        if self.repeat_count.is_none() { self.repeat_count = Some(1); }
        if self.clean_on_error.is_none() { self.clean_on_error = Some(true); }
        if self.max_retries.is_none() { self.max_retries = Some(6); }
        if self.rollover.is_none() { self.rollover = Some(Rollover::Enforce0); }
